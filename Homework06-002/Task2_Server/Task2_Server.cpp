...
int SEND_IP(SESSION session, int flag) {
	...
	if (session.status == 1) {
		char* temp = new char[3]{ "11" };
		ret = SEND_TCP(session.connSock, AddHeader(dest, temp), flag);
		if (ret == SOCKET_ERROR) return SOCKET_ERROR;
		list<char*>::iterator pointer = session.address.begin();
		for (; pointer != session.address.end(); pointer++) {
...
void ResolverDomainName(char* name, SESSION* session) {
	...
	else
	{
		...
		while (infoHostName != NULL) {
			...
			session->address.push_back(cTemp);
		...
		for (int i = 0; i < FD_SETSIZE; i++) {
			...
			if (ret <= 0) {
				...
				if (IsDomainNameOrIP(buff)) {
					...
					ret = SEND_DOMAIN_NAME(client[i], 0);
					if (ret == SOCKET_ERROR) printf("cannot send\n");
				}
			}
		}
