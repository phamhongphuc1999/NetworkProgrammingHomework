...
//list of account taken from file
vector<string> accountList;

//list of session
list<SESSION*> listSession;

int lockAccount, lockSession;
...
vector<string> CreateAccountList() {
	...
	if (session->type == 0) return new char[4]{ "101" };
		...
		if (cmpPass) {
			...
			if (session->numberOfError > 3) {
				...
				int length = accountList[session->location].length();
				accountList[session->location][length - 1] = '1';
...
char* LOGIN(SESSION* session, char* username, char* password) {
	string line;
	int check = 1, length = accountList.size(), index = 0;
	for (int i = 0; i < length; i++) {
		line = accountList[i]; index = i;
...
unsigned _stdcall CreateAsynchronousAccountList(void* param) {
	while (true) {
		if (lockAccount == 0) {
			lockAccount = 1;
			accountList.clear();
...
unsigned _stdcall UpdateAsynchronousFile(void* param) {
	while (true)
	{
		if (lockAccount == 0) {
			lockAccount = 1;
...
unsigned _stdcall Handler(void* param) {
	...
	try {
		_beginthreadex(0, 0, CreateAsynchronousAccountList, 0, 0, 0);
	}
	catch (exception) {
	...
	while (true) {
		...
		if (ret == SOCKET_ERROR) {
			...
			HANDLE handler[2];
			handler[0] = (HANDLE)_beginthreadex(0, 0, UpdateAsynchronousFile, 0, 0, 0);
			handler[1] = (HANDLE)_beginthreadex(0, 0, ReleaseSession, (void*)&session, 0, 0);
			WaitForMultipleObjects(2, handler, true, INFINITE);
